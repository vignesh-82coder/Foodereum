...
class Notification {     
	...
	function saveNotification(){	
		$insertQuery = "
			INSERT INTO ".$this->notificationTable."( `title`, `message`, `ntime`, `repeat`, `nloop`, `username`, `sender`)
			VALUES(?,?,?,?,?,?,?)";
		$stmt = $this->conn->prepare($insertQuery);			
		$stmt->bind_param("sssiiss",$this->title, $this->message, $this->ntime, $this->repeat, $this->nloop, $this->username, $this->sender);
		if($stmt->execute()){
			return true;
		}	 
		return false;				
